...
def _get_flydsl_flex_attention_backward_config(
    ...
    *,
    key: TensorBox | None = None,
    value: TensorBox | None = None,
    ...
    if input_buffers_require_grads(
        ...
        return None, "NYI: FlyDSL flex bwd doesn't support captured grads yet."

    if joint_outputs is not None:
        if joint_outputs.captured_grads_compute:
            return None, "NYI: FlyDSL flex bwd doesn't support captured grads yet."
        if joint_outputs.mutated_grads:
            return None, "NYI: FlyDSL flex bwd doesn't support mutated grads yet."
