...
Partial blocks evaluate the same bounded mask bytecode as the FlyDSL forward.
That keeps causal, document, and sliding-window semantics aligned across both
directions, including captured int32 mask buffers.
"""
    ...
    *,
    ...
    short_packed_causal = (
        ...
        and (
            ...
            or (B == 4 and H in (16, 128) and batched_causal_document_mask)
        )
    )
    return _build_flex_attn_bwd_module(
        ...
        dq_reduction_rows=32 if short_packed_causal else 64,
    )
...
def _build_flex_attn_bwd_module(
    ...
    *,
    ...
    if DQ_BN not in (32, 64) or BN % DQ_BN:
